...
test("a fresh waiting duplicate renews expiry without changing queue order", () => {
  const current = notification({ id: 1, variant: "error", message: "current" })
  ...
  const later = notification({ id: 3, variant: "error", message: "later" })
  const duplicate = notification({ id: 4, variant: "error", message: "queued" })

  const next = enqueueNotification([current, queued, later], duplicate, NOW)

  assert.deepEqual(next, [current, { ...queued, expiresAt: FUTURE }, later])
  assert.equal(next[0], current)
  assert.equal(queued.expiresAt, NOW + 1)
  assert.deepEqual(
    advanceNotificationQueue(next, NOW + 2).map(({ id }) => id),
    [2, 3]
  )
  assert.deepEqual(advanceNotificationQueue(next, FUTURE), [])
})
