...
function enqueueNotification(
  ...
  const duplicateIndex = freshNotifications.findIndex((queued) =>
    sameNotification(queued, notification)
  )
  if (duplicateIndex !== -1) {
    return duplicateIndex === 0
      ? freshNotifications
      : freshNotifications.map((queued, index) =>
          index === duplicateIndex
            ? { ...queued, expiresAt: notification.expiresAt }
            : queued
        )
  }
