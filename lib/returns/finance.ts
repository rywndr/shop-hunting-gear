...
export function confirmedOrderRefundSql() {
  return sql<number>`greatest(0,
    ...
    - coalesce((SELECT sum(c.refund_amount) FROM order_cancellation c
      WHERE c.order_id = "customer_order"."id" AND c.financial_action = 'refund'
        AND c.status = 'refund_pending' AND c.reconciliation_status = 'reconciled'), 0)
